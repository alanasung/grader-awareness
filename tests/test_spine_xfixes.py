...
def test_mps_fallback_flag_in_device_info_dict():
    from graderaw.models.device import DeviceInfo
...
def test_enable_mps_fallback_sets_env(monkeypatch):
    from graderaw.models import device as device_mod
...
def test_apply_chat_template_raw_path():
    from graderaw.models.generation import apply_chat_template
...
def test_role_keyed_config():
    from graderaw.configs.schema import Config, ModelConfig
...
def test_data_default_n_items_pilot_sized():
    from graderaw.configs.schema import DataConfig, EvalConfig
...
def test_validate_layers_rejects_gpt2_layer_12():
    from graderaw.utils.validation import validate_layers
...
def test_registry_revision_pinned():
    from graderaw.models.registry import MODEL_REGISTRY, get_model_spec
...
def test_mde_and_tost():
    from graderaw.evaluation.metrics import Estimate, minimum_detectable_effect, tost_equivalence
...
def test_patch_activations_and_attn_exported():
    from graderaw.models.hooks import intervene_attention_heads, patch_activations
...
def test_ablation_controls_and_dropout():
    from graderaw.ablation import run_component_dropout, run_control_arms, run_seed_sweep
...
def test_reporting_roundtrip(tmp_path):
    ...
    from graderaw.reporting import aggregate_results, write_figures, write_tables
    from graderaw.utils.run_manifest import write_json_atomic
