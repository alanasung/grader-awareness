# Design Document: When Models Notice the Grader During Training

## 1. Purpose

This document is the method contract for **When Models Notice the Grader During Training**. It specifies what is
...
One-liner: Detect and intervene on signals that a training grader is present.

Hypothesis: Detect and intervene on signals that a training grader is present.
...
|---|---|---|
| Config schema | `graderaw.configs.schema` | Typed dataclasses + role-keyed models |
| Loader | `graderaw.configs.loader` | Hydra compose API |
| Device | `graderaw.models.device` | MPS/CUDA/CPU resolution + fallback flag |
| Registry | `graderaw.models.registry` | Pinned revisions + hardware fit |
| Hooks | `graderaw.models.hooks` | Capture, steer, ablate, patch, attn intervene |
| Generation | `graderaw.models.generation` | Chat template path recording |
| Data | `graderaw.data` | Synthetic items, splits, manifests |
| Evaluation | `graderaw.evaluation` | Metrics, failure harness, protocols |
| Ablation | `graderaw.ablation` | Control / dropout / seed-sweep arms |
| Reporting | `graderaw.reporting` | Aggregate → MD/TeX/figures |
| Cache | `graderaw.cache` | Versioned artifact store |
...
`src/graderaw/graderaw/` are preserved across regeneration.
